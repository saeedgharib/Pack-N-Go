...
const EditInventory = () => {
 let ImageUrl
  ...
  const [formData, setFormData] = useState({
    ...
    moverId: "1617447942983",
    moveDate: "1617447942983",
    moveTime: "",
    itemsDescription: "",
    moversRequired: "",
    specialRequests: "",
  ...
  const fetchOrder =async()=>{
    ...
    try { 
      ...
      // const docRef = doc(DB,"inventory", ID);
      const docSnap = await getDoc(doc(DB,"JobListing",orderId));
  ...
  console.log("formdate",formData.moveDate);
  
const dat =new Date(1617447942983);
console.log("formdate",dat.toLocaleDateString());
...
{/* <Image source={{ uri: image }} style={styles.image} /> */}
 ...
 <Spinner visible={loading} />
    <ScrollView showsVerticalScrollIndicator={false}>
      <TouchableOpacity onPress={pickImage} style={styles.imagePicker}>
        {formData ? (
          <Image src={formData.image} style={styles.image} />
        ) : (
          <Text style={styles.imagePlaceholder}>Tap to select an image</Text>
        )}
      </TouchableOpacity>
      ...
      <View style={styles.buttonContainer}>
        <TextInput
          ...
          placeholder={dat.toLocaleDateString()|| "Select a date"}
